...
if (process && process.versions && process.versions.electron) {
  try {
    ...
    storagePath = userData
      ? path.join(userData, "crm.sqlite")
      : path.join(defaultDevDataDir, "crm.sqlite");
  } catch (e) {
...
const JobSheet = sequelize.define("JobSheet", {
  ...
  currentStep: {
    ...
    defaultValue: "melting",
  },
  status: {
    type: DataTypes.ENUM("Issued", "In-Progress", "Completed", "Pending"),
    defaultValue: "Issued",
  },
...
const JobSheetStep = sequelize.define("JobSheetStep", {
  stepName: {
    type: DataTypes.ENUM("melting", "rolling", "press", "TPP", "packing"),
    allowNull: false,
  },
...
const Inventory = sequelize.define("Inventory", {
  ...
  category: { type: DataTypes.STRING }, // "Gold", "Silver"
  quantity: { type: DataTypes.FLOAT, allowNull: false, defaultValue: 0 },
  unit: { type: DataTypes.STRING, defaultValue: "gm" }, // "KG", "gram"
  costPrice: { type: DataTypes.FLOAT, allowNull: false, defaultValue: 0 }, // Buying Price
  sellingPrice: { type: DataTypes.FLOAT, allowNull: false, defaultValue: 0 }, // Current Price
  reorderLevel: { type: DataTypes.FLOAT, defaultValue: 10 },
  supplier: { type: DataTypes.STRING }, // Stores BullionMerchant Name or ID
  paymentType: { type: DataTypes.STRING }, // "cash", "rtgs", "neft", "cheque"
...
async function initDb() {
  ...
  // FORCE SYNC INVENTORY to apply new columns (since User allowed DB changes)
  // WARNING: This drops the Inventory table and recreates it.
  await models.Inventory.sync({ alter: true });
  await models.BullionMerchant.sync({ alter: true });
