...
router.get("/", async (req, res) => {
  ...
  res.render("production/jobsheets/list", {
    title: "Job Sheets",
    jobsheets,
    employees,
    filters: { date, employeeId, status },
  });
});
...
router.get("/dashboard", async (req, res) => {
  ...
  // Loss today
  const lossToday =
    (await models.JobSheet.sum("totalLoss", {
  ...
  // Scrap this month
  const scrapMonth =
    (await models.JobSheet.sum("scrapWeight", {
      where: { issueDate: { [Op.gte]: monthStart } },
    })) || 0;

  // Dust this month
  const dustMonth =
    (await models.JobSheet.sum("dustWeight", {
      where: { issueDate: { [Op.gte]: monthStart } },
    })) || 0;
...
router.get("/:id/complete-step", async (req, res) => {
  ...
  if (jobsheet.status === "Completed") {
    return res.redirect(
      `/jobsheets/${jobsheet.id}?message=Job already completed`,
    );
  }

  const currentStep = jobsheet.JobSheetSteps.find(
    (s) => s.status === "in-progress",
  );
  const employees = await models.Employee.findAll({ order: [["name", "ASC"]] });

  // Verify no steps are skipped (only check up to the current active step)
  const allSteps = jobsheet.JobSheetSteps.sort(
  ...
  if (currentStep) {
    ...
    const incompletePrevious = previousSteps.find(
    ...
    if (incompletePrevious) {
      return res.status(400).render("production/jobsheets/complete-step", {
        ...
        currentStep: null,
        employees,
        error: `Previous steps must be completed first (Step ${incompletePrevious.stepOrder} is ${incompletePrevious.status})`,
      });
    }
  }
...
router.post("/:id/complete-step", async (req, res) => {
  ...
  try {
    ...
    const previousSteps = jobsheet.JobSheetSteps.filter(
      (s) => s.stepOrder < currentStep.stepOrder,
    );
    const incompletePrevious = previousSteps.find(
      (s) => s.status !== "completed",
...
router.post("/:id/start-next-step", async (req, res) => {
  try {
    ...
    const completedSteps = jobsheet.JobSheetSteps.filter(
      (s) => s.status === "completed",
    ...
    const nextStep = jobsheet.JobSheetSteps.find(
      (s) =>
